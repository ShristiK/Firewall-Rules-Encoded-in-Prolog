...
check_Src(X):-       isl2(L2),not(member(X,L2)),pow2(2,128,Z),not(between(1,Z,X)),write('not correct src address').
check_Dest(X):-      isl2(L2),not(member(X,L2)),pow2(2,128,Z),not(between(1,Z,X)),write('not correct dest address').
